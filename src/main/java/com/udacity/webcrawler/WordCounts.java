...
final class WordCounts {
  ...
  static Map<String, Integer> sort(Map<String, Integer> wordCounts, int popularWordCount) {
    ...
    // Takes the passed in map converts to stream, sorts with the existing comparator,
    // shrinks set to the passed length the collects to a linkedHashMap to preserve order
    return wordCounts.entrySet()
            .stream()
            .sorted(new WordCountComparator())
            .limit(popularWordCount)
            .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue,
                    (origKey, newKey) -> origKey, LinkedHashMap::new));
  }
